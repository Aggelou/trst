package com.soft.trst.web.rest;
...
import com.soft.trst.config.KafkaProperties;
...
class TrstKafkaResourceIT {
    ...
    void setup() {
        ...
        TrstKafkaResource kafkaResource = new TrstKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/trst-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/trst-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
